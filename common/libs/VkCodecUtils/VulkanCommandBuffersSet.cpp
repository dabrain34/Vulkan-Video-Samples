...
VkResult VulkanCommandBuffersSet::CreateCommandBufferPool(const VulkanDeviceContext* vkDevCtx,
                                                          ...
                                                          uint32_t maxCommandBuffersCount)
{
    ...
    m_vkDevCtx = vkDevCtx;
     ...
     if (result != VK_SUCCESS) {
         VKVS_FAIL("ERROR: Can't allocate command buffer pool");
         return result;
     }
     ...
     if (result != VK_SUCCESS) {
         VKVS_FAIL("ERROR: Can't get command buffer");
     }
     return result;
}
