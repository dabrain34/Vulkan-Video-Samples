...
{
    LOG_S_DEBUG << std::endl << "Input order:   ";
    for (uint64_t frameNum = 0; frameNum < numFrames; frameNum++) {
        LOG_S_DEBUG << std::setw(3) << frameNum << " ";
    }
    LOG_S_DEBUG << std::endl << "Frame Type:   ";
    ...
    for (uint64_t frameNum = 0; frameNum < (numFrames - 1); frameNum++) {
        ...
        LOG_S_DEBUG << std::setw(4) << GetFrameTypeName(gopPos.pictureType);
    }
    GetPositionInGOP(gopState, gopPos, false, true);
    LOG_S_DEBUG << std::setw(4) << GetFrameTypeName(gopPos.pictureType);

    LOG_S_DEBUG << std::endl << "Encode  order: ";

    gopState = GopState();
    for (uint64_t i = 0; i < (numFrames - 1); i++) {
        GetPositionInGOP(gopState, gopPos);
        LOG_S_DEBUG << std::setw(3) << gopPos.encodeOrder << " ";
    }
    GetPositionInGOP(gopState, gopPos, false, true);
    LOG_S_DEBUG << std::setw(3) << gopPos.encodeOrder << " ";
