...
class VulkanVideoTestFrameworkBase:
    ...
    def __init__(self, executable_path: str = None, **options):
        ...
        # Handle deny list options
        ignore_deny_list = options.get('ignore_deny_list', False)
        only_denied = options.get('only_denied', False)
        if only_denied:
            deny_filter = DenyFilter.DENIED
        elif ignore_deny_list:
            deny_filter = DenyFilter.ALL
        else:
            deny_filter = DenyFilter.ENABLED

        # Load deny list
        deny_list_path = options.get('deny_list')
        self._deny_rules: List[DenyRule] = []
        if deny_filter != DenyFilter.ALL:
            try:
                self._deny_rules = load_deny_list(deny_list_path)
            except (OSError, json.JSONDecodeError) as e:
                print(f"⚠️  Failed to load deny list: {e}")
        ...
        self._options = {
            ...
            'show_denied': options.get('show_denied', False),
        }
    ...
    def deny_filter(self) -> DenyFilter:
        """Deny filter mode (ENABLED, DENIED, or ALL)."""
        return self._options['deny_filter']

    @property
    def deny_rules(self) -> List[DenyRule]:
        """List of deny rules loaded from deny list."""
        return self._deny_rules

    @property
    def show_denied(self) -> bool:
        """Whether to show denied tests in summary output."""
        return bool(self._options['show_denied'])

    @property
    def ignore_deny_list(self) -> bool:
        """Whether to ignore the deny list and run all tests."""
        return self.deny_filter == DenyFilter.ALL

    @property
    def only_denied(self) -> bool:
        """Whether to run only denied tests."""
        return self.deny_filter == DenyFilter.DENIED
    ...
    def _count_denied_tests(self, samples: list, test_format: str = "vvs",
                            test_type: str = "decode") -> int:
        """Count denied tests from samples list based on deny rules"""
        count = 0
        for sample in samples:
            deny_rule = is_test_denied(
                sample.name, test_format, self._deny_rules,
    ...
    def _count_non_denied_tests(self, samples: list, test_format: str = "vvs",
                                test_type: str = "decode") -> int:
        """Count non-denied tests from samples list"""
        return len(samples) - self._count_denied_tests(
    ...
    def print_summary(self, results: List[TestResult] = None,
        ...
        # Count denied/non-denied tests based on filter mode
        denied_count = 0
        non_denied_count = 0
        if self.deny_filter == DenyFilter.DENIED and all_samples:
            # In DENIED mode, show how many non-denied tests are skipped
            non_denied_count = self._count_non_denied_tests(all_samples)
        elif self.deny_filter == DenyFilter.ENABLED and all_samples:
            # In ENABLED mode, show how many denied tests are skipped
            denied_count = self._count_denied_tests(all_samples)
        # When DenyFilter.ALL, don't count denied/non-denied separately

        total_including_all = len(results) + denied_count + non_denied_count
        print(f"Total Tests:   {total_including_all:3}")
        if self.deny_filter == DenyFilter.DENIED and non_denied_count > 0:
            print(f"Non-denied (skipped): {non_denied_count:3} "
                  "(--only-denied mode)")
        elif self.deny_filter == DenyFilter.ENABLED and denied_count > 0:
            print(f"Denied:        {denied_count:3} "
                  "(use --ignore-deny-list to run)")
        # Don't show denied count when using --ignore-deny-list (ALL mode)
    ...
    def filter_test_suite(
        ...
        deny_filter: DenyFilter = DenyFilter.ENABLED,
        test_format: str = "vvs",
        test_type: str = "decode",
    ) -> list:
        """Filter test samples based on codec, pattern, and deny list

        Args:
            samples: List of test samples to filter
            codec_filter: Filter by codec type
            test_pattern: Filter by name pattern
            deny_filter: Filter mode (ENABLED, DENIED, or ALL)
            test_format: Test format for deny list matching ('vvs', 'fluster',
                        'soothe')
            test_type: Test type for deny list matching ('decode', 'encode')
        """
        filtered_samples = []
        for sample in samples:
            ...
            is_denied = deny_rule is not None

            # Handle deny filtering
            # But allow exact matches to override deny filtering
            if deny_filter == DenyFilter.DENIED and not is_denied and not exact_match:
                # Skip non-denied samples when filter is DENIED, unless exact
                continue
            if deny_filter == DenyFilter.ENABLED and is_denied and not exact_match:
                # Skip denied samples when filter is ENABLED, unless exact match
                continue
            # DenyFilter.ALL includes both denied and non-denied
...
def run_complete_framework_main(framework_class, test_type: str, args) -> int:
    ...
    if test_type == "decoder":
        framework = framework_class(
            ...
            ignore_deny_list=args.ignore_deny_list,
            only_denied=args.only_denied,
            show_denied=args.show_denied,
