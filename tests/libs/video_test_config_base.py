...
class DenyFilter(Enum):
    """Enumeration for filtering denied tests"""
    ENABLED = "enabled"      # Only non-denied tests (default)
    DENIED = "denied"        # Only denied tests
    ALL = "all"              # Both denied and non-denied tests


@dataclass
class DenyRule:
    """
    Rule for denying (skipping) a test based on various conditions.

    A test is denied if it matches a rule where:
    ...
    Attributes:
        ...
        drivers: List of GPU drivers to deny. Valid values:
                 'all', 'nvidia', 'nvk', 'intel', 'anv', 'amd', 'radv'
        platforms: List of platforms to deny ('all', 'windows', 'linux')
                   Note: Platform filtering is defined but not enforced.
        reproduction: Whether failure is consistent ('always', 'flaky')
        reason: Human-readable explanation for the deny
        bug_url: Link to tracking issue
        date_added: When the deny was added (YYYY-MM-DD format)
    """
    name: str
    test_type: str
    format: str
...
def load_deny_list(deny_list_path: Optional[str] = None) -> List[DenyRule]:
    """
    Load deny list from JSON file.

    Args:
        deny_list_path: Path to deny list JSON file.
                       If None, uses default 'denied_samples.json' in tests dir.

    Returns:
        List of DenyRule objects

    Raises:
        FileNotFoundError: If deny list file doesn't exist
        json.JSONDecodeError: If file is not valid JSON
    """
    if deny_list_path is None:
        # Default to denied_samples.json in tests directory
        deny_list_path = str(Path(__file__).parent.parent / "denied_samples.json")

    json_path = Path(deny_list_path)
    if not json_path.is_absolute():
        # Try relative to tests directory
        if not json_path.exists():
            json_path = Path(__file__).parent.parent / deny_list_path

    if not json_path.exists():
        # Return empty list if no deny list file exists
        return []
    ...
    deny_rules = []
    for entry in data.get('denied_tests', []):
        rule = DenyRule(
            name=entry.get('name', ''),
...
def is_test_denied(
    ...
    """
    Check if a test should be denied based on the deny list.

    A test is denied if ANY rule in the deny list matches.
    ...
    Args:
        ...
        deny_rules: List of DenyRule objects to check against
        current_driver: Current GPU driver name (default: 'all' to match all)
        test_type: Type of the test ('decode', 'encode')

    Returns:
        The matching DenyRule if the test is denied, None otherwise
    """
    for rule in deny_rules:
        ...
        # All conditions match - test is denied
        return rule
