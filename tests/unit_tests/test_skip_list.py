...
class TestIsTestSkipped:
    ...
    def test_wildcard_pattern_star(self):
        ...
        rules = [SkipRule(name="av1_*_10bit", test_type="decode", format="vvs")]

        # Should match
        result = is_test_skipped("av1_basic_10bit", "vvs", rules, test_type="decode")
        assert result is not None

        result = is_test_skipped("av1_advanced_10bit", "vvs", rules, test_type="decode")
        assert result is not None

        # Should not match
        result = is_test_skipped("av1_basic_8bit", "vvs", rules, test_type="decode")
        assert result is None

    def test_wildcard_pattern_question(self):
        """Test wildcard pattern with question mark"""
        rules = [SkipRule(name="h264_clip_?", test_type="decode", format="vvs")]

        # Should match single character
        result = is_test_skipped("h264_clip_a", "vvs", rules, test_type="decode")
        assert result is not None

        result = is_test_skipped("h264_clip_b", "vvs", rules, test_type="decode")
        assert result is not None

        # Should not match multiple characters
        result = is_test_skipped("h264_clip_ab", "vvs", rules, test_type="decode")
        assert result is None
...
class TestLoadSkipList:
    ...
    def test_load_from_valid_json(self):
        """Test loading skip list from valid JSON file"""
        skip_data = {
            "version": "1.0",
            "skipped_tests": [
                {
    ...
    def test_load_empty_skipped_tests(self):
        """Test loading JSON with empty skipped_tests array"""
        skip_data = {"skipped_tests": []}
