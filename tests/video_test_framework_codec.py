...
class VulkanVideoTestFramework:  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, encoder_path: Optional[str] = None,
        ...
        self.deny_list_path = options.get('deny_list', 'denied_samples.json')
        self.ignore_deny_list = options.get('ignore_deny_list', False)
        self.only_denied = options.get('only_denied', False)
        self.show_denied = options.get('show_denied', False)

        # Determine deny filter mode
        if self.ignore_deny_list:
            self.deny_filter = DenyFilter.ALL
        elif self.only_denied:
            self.deny_filter = DenyFilter.DENIED
        else:
            self.deny_filter = DenyFilter.ENABLED

        # Load deny rules
        self.deny_rules = load_deny_list(self.deny_list_path)
    ...
    def _count_denied_tests(self) -> int:
        """Count denied tests from both frameworks using deny list"""
        total_denied = 0
        if self.encode_framework and self.encode_framework.encode_samples:
            for sample in self.encode_framework.encode_samples:
                if is_test_denied(sample.name, "vvs", self.deny_rules,
                                  test_type="encode"):
                    total_denied += 1
    ...
    def _count_non_denied_tests(self) -> int:
        """Count non-denied (enabled) tests from both frameworks"""
        total_enabled = 0
        if self.encode_framework and self.encode_framework.encode_samples:
            for sample in self.encode_framework.encode_samples:
                if not is_test_denied(sample.name, "vvs", self.deny_rules,
                                      test_type="encode"):
                    total_enabled += 1
        if self.decode_framework and self.decode_framework.decode_samples:
            for sample in self.decode_framework.decode_samples:
    ...
    def print_summary(self, encode_results: Optional[List] = None,
        ...
        # Count denied tests for display and total calculation
        total_denied = self._count_denied_tests()

        # Calculate total_including_denied based on mode
        if self.ignore_deny_list:
            # When --ignore-deny-list is used, denied tests are already
            # included in all_results, so don't add them again
            total_including_denied = total_tests
        elif self.only_denied:
            # When --only-denied is used, only denied tests were run,
            # so add non-denied tests to show total suite size
            total_including_denied = total_tests + self._count_non_denied_tests()
        else:
            # Normal mode: total_tests is non-denied tests, add denied count
            total_including_denied = total_tests + total_denied

        print("\n" + "=" * 70)
        print("OVERALL SUMMARY")
        print("=" * 70)
        print(f"Total Tests:   {total_including_denied:3}")
        if self.only_denied:
            # When running only denied tests, show how many enabled were
            # skipped
            non_denied = self._count_non_denied_tests()
            if non_denied > 0:
                print(f"Skipped (enabled): {non_denied:3} "
                      "(remove --only-denied to run)")
        elif not self.ignore_deny_list and (total_denied > 0 or
                                            self.show_denied):
            # Show denied count when not using --ignore-deny-list
            print(f"Denied:        {total_denied:3} "
...
def list_all_samples(deny_list_path: str = "denied_samples.json") -> None:
    ...
    if decoder_samples:
        ...
        for sample in decoder_samples:
            ...
            # Check if test is denied
            if is_test_denied(sample['name'], "vvs", deny_rules,
                              test_type="decode"):
                name = f"[DENIED] {name}"
                denied_decode_count += 1
            print(f"{name:<40} {codec:<8} {description}")
    else:
    ...
    total_denied = denied_decode_count + denied_encode_count

    print(f"\nTotal: {total_count} samples "
          f"({decoder_count} decoder, {encoder_count} encoder)")
    if total_denied > 0:
        print(f"Denied: {total_denied} samples "
              f"(use --ignore-deny-list to run all)")
...
def create_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--deny-list",
        help="Path to custom deny list JSON file (default: denied_samples.json)")
    parser.add_argument(
        "--ignore-deny-list", action="store_true",
        help="Ignore the deny list and run all tests")
    parser.add_argument(
        "--only-denied", action="store_true",
        help="Run only denied tests (excludes non-denied tests)")
    parser.add_argument(
        "--show-denied", action="store_true",
        help="Show denied tests in summary output")
    parser.add_argument(
        "--encode-test-suite",
...
def main() -> int:
    ...
    if args.list_samples:
        deny_list_path = args.deny_list or "denied_samples.json"
        list_all_samples(deny_list_path)
        return 0
